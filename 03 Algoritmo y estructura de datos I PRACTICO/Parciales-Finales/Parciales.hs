...
-- Errores de mi parcial:
-- que no me corrija esa profe por que obvio me va mal, la odio. 

-- En general:
--1) Separar la hipotesis inductiva de la especificacion. 
--2) Ir mas despacio. 
--3) Realmente pensar logicamente, el decir "que hice aca?". Un explicarme mas lo que hago. 
--4) Evaluar mejor.
--5) Cuando generalizo, debo saber que me olvido de la funcion del inicio, por que no sirve, sin embargo debo de hacer la igualdad (o sea, que vale cada cosa)
--6) paciencia, porfavor. 
--7) Cuando generalizo, debo de decir que la generalizacion es valida para un n, o sea:
--8) CUANTIFICAR, SI PONGO QUE AS, ES A:AS, CUANTIFICAR TAMBIEN LA A. 
{-

    Mi Hipotesis inductiva:
    gF.E.xs = ⟨Max i : 0 ≤ i < #xs ∧ E + sum.(xs↑i) = sum.(xs↓i) : i ⟩
    Mi especificacion:
    gF.n.xs = ⟨Max i : 0 ≤ i < #xs ∧ n + sum.(xs↑i) = sum.(xs↓i) : i ⟩
    
    tq, cuando empiezo, digo:
...
-- Hoja 3. -> A la hora de hacer la evaluacion, aclarar los resultados finales. Por eso la marca. 

-- Hoja 4. ->  Aca entra en practica lo que dije en el punto 7, en adicion, el problema mio de la generalizacion. 
-- Ver como se evalua eso, deberia de dar 67 o algo parecido, asi k chekiar. 

-- Hoja 5. -> nada.

-- Hoja 6. -> Fijarse que cuando separo, y tengo un sum, usar el + y no seguir con el v, aparentemente es algo logico)? 
-- y repetir lo que uso aca, por que use sum(x:xs) y despues en la generalizacion, en vez de seguir usandolo, lo cambio a sum.bs y nunca iguale a el 0 que esta ahi. 

-- Hoja 7. -> Aplica lo de hoja 6, ademas detalar el paso que esta marcado. 

-- Hoja 8. -> Aplica error 8. 
