...
**Idea clave:** el shell es un _cliente_ del kernel. Todo lo que el shell "hace" en términos de crear procesos, redirigir E/S o comunicar comandos, en realidad lo pide al kernel a través de **syscalls** (`fork`, `execvp`, `pipe`, `open`, `dup2`, `wait`, etc.). El shell traduce; el kernel ejecuta.
...
|Fase|Tarea|
|---|---|
|Read|Leer la línea de entrada del usuario (parsear el texto)|
...
> **Nota importante para el examen:** este `mybash.c` está incompleto a propósito (`// COMENTADO PARA MEMTEST` y `/* COMPLETAR */`). Es el esqueleto que hay que completar: falta invocar `parse_pipeline()` y, según si el pipeline es un builtin solitario (`builtin_alone`) o no, llamar a `builtin_run()` o a `execute_pipeline()`. Es un buen candidato a pregunta de "completar código".
...
**Razón de ser de los builtins:** `cd` cambia el directorio de trabajo del proceso que lo ejecuta (`chdir()`). Si `cd` se ejecutara como proceso hijo (fork+exec), cambiaría el directorio _del hijo_, que muere inmediatamente después — el shell padre nunca vería el cambio. Por eso `cd` **tiene** que ejecutarse en el mismo proceso del shell. Lo mismo pasa con `exit` (tiene que terminar el proceso del shell, no uno hijo) y con `export` (variables de entorno del propio shell).
...
> **Importante:** en el código de `execute.c` que fue provisto, `execute_pipeline()` **siempre** espera a todos los hijos con el `for` final de `wait(NULL)`, sin condicionar por `pipeline_get_wait(apipe)`. Es decir, tal como está el código, **falta** el `if (pipeline_get_wait(apipe)) { ... }` alrededor del bucle de `wait()` para que el background realmente no bloquee al shell. Es un buen punto para revisar/discutir en el examen: la lógica de _foreground vs background_ está modelada en el TAD (`pipeline_get_wait`), pero en `execute.c` provisto no se usa todavía para condicionar el `wait()`.
...
> **Nota sobre el código real:** el `execute.c` provisto **solo implementa `<` y `>`** (usa `O_WRONLY | O_CREAT | O_TRUNC`, que siempre trunca el archivo — no hay lógica para `O_APPEND` ni para redirigir `STDERR_FILENO`). El TAD `scommand` en `command.h` también solo tiene `redir_in` y `redir_out` (no hay campo para stderr). Si el examen pregunta por `>>` o `2>`, hay que responder a nivel conceptual (qué flags de `open()` usarían: `O_APPEND` para `>>`, `dup2(fd, STDERR_FILENO)` para `2>`), aclarando que el código de este laboratorio no los implementa.
...
Funciones de GLib usadas: `g_list_append()` (agregar al final), `g_list_length()` (longitud), `g_list_delete_link()` (borrar un nodo puntual), `g_list_free()` / `g_list_free_full()` (liberar toda la lista, esta última liberando también el contenido con una función, en este caso `free`). GLib es la herramienta **sugerida** por la cátedra para no reinventar listas enlazadas a mano en los TADs.
...
- **`parser`** (dado, `parser.o` + `lexer.o`) es el TAD de **bajo nivel**: sabe leer caracteres del `FILE *` de entrada y reconocer _tokens_ (un argumento normal, una redirección `<`/`>`, el operador `|`, el operador `&`, fin de línea, basura). No sabe nada de `scommand` ni `pipeline`.
...
- `mybash.c` está incompleto: falta invocar `parse_pipeline`, decidir builtin vs. no builtin, y llamar a `builtin_run`/`execute_pipeline` según corresponda.

---

## 7. Laboratorio 0 — Herramientas de Línea de Comandos (Shell Scripting)

> Este laboratorio es previo a MyBash y no trata de _implementar_ un shell, sino de **usarlo** como usuario: encadenar herramientas Unix con pipes para resolver problemas de procesamiento de texto/datos. Es útil para el examen porque son los mismos comandos externos que MyBash tiene que poder ejecutar (`grep`, `wc`, `sort`, etc.) y porque refuerza la idea de pipe (`|`) desde el lado del usuario.
...
|`wc -l`|Cuenta líneas de la entrada (`wc` = _word count_; `-l` = contar líneas en vez de palabras/bytes)|
...
|Comando|Qué hace|
|---|---|
|`curl -s URL`|Descarga el contenido de una URL e imprime el body por stdout; `-s` = _silent_ (no muestra la barra de progreso)|
...
|`tr 'A-Z' 'a-z'`|_Translate_: reemplaza carácter a carácter el primer conjunto por el segundo (acá, pasa todo a minúsculas)|
|`sed -e 'expr1' -e 'expr2' ...`|_Stream editor_: aplica una o más expresiones de edición línea por línea. Cada `-e` es una expresión distinta|
|`sed 's/ /_/g'`|Sustitución (`s/patrón/reemplazo/flags`): reemplaza espacios por guiones bajos; `g` = _global_ (todas las ocurrencias de la línea, no solo la primera)|
...
## Preguntas abiertas para vos (para completar/corregir esta guía)

- ¿La presentación o el video mencionan explícitamente `>>` o `2>` como parte del alcance del examen, más allá de lo que implementa el código?
- ¿La cátedra espera que sepan de memoria las firmas exactas de `open()` (flags como `O_CREAT`, `O_TRUNC`, `O_APPEND`, permisos `0644`) o alcanza con la idea general?
- ¿Quieren que agregue algo sobre `export` (mencionado en la consigna como builtin) aunque no está implementado en el `builtin.c` subido?